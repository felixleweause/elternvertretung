...
import type { HomeSnapshot } from "@/lib/react-query/query-functions";
import { QUICK_ACTIONS, ROLE_LABELS, type UserRole } from "@/lib/constants/home";
import { useHomeQuery } from "@/lib/react-query/hooks";

type HomeProps = {
  schoolId: string;
  fallbackData: HomeSnapshot;
  userName: string;
};

export function Home({ schoolId, fallbackData, userName }: HomeProps) {
  const query = useHomeQuery(schoolId);
  ...
  return (
    <section className="space-y-8">
      <div className="relative overflow-hidden rounded-3xl border border-zinc-200 bg-gradient-to-br from-zinc-50 via-white to-zinc-100 p-8 shadow-lg dark:border-zinc-800 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950">
        <div className="absolute inset-y-0 -right-36 h-[160%] w-[60%] rounded-full bg-gradient-to-tr from-blue-500/10 via-emerald-500/10 to-purple-500/10 blur-3xl dark:from-blue-600/20 dark:via-emerald-600/20 dark:to-purple-500/20" />
